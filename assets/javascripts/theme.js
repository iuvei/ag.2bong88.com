...
(function() {
	...
	$(document).ready(function(){
		...
		$("#re-check-order").click(function(){
			...
			$.ajax({
				...
				type: 'POST',
				url: '/CardManamentAdmin.php?r=order/ReCheck',
				data: "id="+id,
  ...
  $(document).ready(function() {
	...
	else
	{
		...
		idMenuOpened = $.cookie('menu-open');
		$("#menu-"+idMenuOpened).addClass('in');
		$("#ul-menu-"+idMenuOpened).addClass('in');
		$("#ul-menu-"+idMenuOpened).css('display','block');
		
	
	}
	if($.cookie('menu-child-a')=="")
		$.cookie('menu-child-a',0);
	else
	{
	
		$("#menu-"+$.cookie('menu-child-a')+"-a").css("background","rgb(160, 145, 145)");
	
	}
	
	if($.cookie('menu-no-sub')=="")
		$.cookie('menu-no-sub',0);
	else
	{
	
		$("#menu-no-sub-"+$.cookie('menu-no-sub')).css("background","rgb(213, 206, 206)");
    ...
    $("#main-nav .dropdown-collapse").on(click_event, function(e) {
      ...
      if (list.is(":visible")) {
        ...
        link.addClass("in");
		var idMenu = link.attr('data-id');
		$.cookie('menu-open',idMenu);
	...
	$(".no-dropdown").on(click_event, function(e) {
	
		
		
			$.cookie('menu-open','0');
			$.cookie('menu-no-sub',$(this).attr('data-id'));
	
	});
	$("#main-nav ul li ul li a").click(function(){
		$.cookie('menu-no-sub',0);
		$.cookie('menu-child-a',$(this).attr('data-id'));
...
function locdau() {
	...
	var str = (document.getElementById("name_title").value);// lấy chuỗi dữ liệu nhập vào từ form có tên title
	...
	document.getElementById("seo_url").value = str;// xuất kết quả xữ lý ra keyword
